...
describe('UserService', () => {
  ...
  test('getAllUsers chama repository findAll', async () => {
    ...
    jest.doMock('../../repositories', () => ({
      UserRepository: jest.fn().mockImplementation(() => ({ findAll })),
    }));

    const { UserService } = require('../UserService');
    const service = new UserService({ findAll });
  ...
  test('getUserById não encontrado lança erro', async () => {
    ...
    const { UserService } = require('../UserService');
    const service = new UserService({ findById });
  ...
  test('createUser valida username duplicado e lança erro', async () => {
    ...
    const { UserService } = require('../UserService');
    const service = new UserService({ findByUsername });

    await expect(
      service.createUser({ nome: 'X', username: 'u', senha: '123456', perfil: 'USER' })
    ).rejects.toThrow();
  });
});
